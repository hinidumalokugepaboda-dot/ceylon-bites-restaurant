    ...
    <div className="min-h-screen bg-white text-slate-900 flex">
      ...
      <main className="flex-1 bg-slate-50 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              title={isAudioEnabled ? 'Kitchen order chime active (Click to mute)' : 'Kitchen chime muted (Click to enable)'}
            >
              {isAudioEnabled ? <Volume2 className="w-4 h-4 text-sky-600" /> : <VolumeX className="w-4 h-4 text-slate-400" />}
        ...
        <div className="p-6 space-y-6">
          ...
          {newOrderNotification && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-gradient-to-r from-sky-500/20 via-sky-500/10 to-sky-500/5 border-2 border-sky-400 rounded-xl p-4 shadow-lg animate-pulse">
              <div className="flex items-center gap-3.5">
                <div className="w-11 h-11 rounded-xl bg-sky-600 text-white flex items-center justify-center font-bold shrink-0 shadow">
                  <Bell className="w-6 h-6 animate-bounce" />
                </div>
                <div>
                  <div className="text-base font-extrabold text-sky-950 flex items-center gap-2">
                    <span>New Order Incoming to Kitchen!</span>
                    <span className="px-2 py-0.5 rounded bg-sky-500 text-white text-[10px] font-black uppercase">
                      Action Required
                    </span>
                  </div>
                  <div className="text-xs text-sky-800 font-medium mt-0.5">
                    {newOrders.length > 0
                      ? `${newOrders.length} ticket${newOrders.length > 1 ? 's' : ''} awaiting kitchen acceptance · Latest: Table #${newOrders[0].tableNumber} (Order #${newOrders[0].orderNumber})`
                      : 'New order ticket arrived for kitchen preparation.'}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2 self-end sm:self-auto">
                <button
                  ...
                  className="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-md transition"
                >
                  View Tickets
                </button>
                <button
