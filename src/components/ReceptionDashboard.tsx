...
export const ReceptionDashboard: React.FC = () => {
  ...
  const newOrders = useMemo(() =>
    orders.filter((o) => ['pending_reception', 'payment_pending', 'pending', 'received', 'sent_to_kitchen'].includes(o.status)),
    [orders]
  );
      ...
      <main className="flex-1 bg-slate-50 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              type="button"
              onClick={testCashierChime}
              className="flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-xs font-semibold border border-slate-200 bg-white hover:bg-slate-100 text-slate-700 transition"
        ...
        <div className="p-6 space-y-6">
          {/* Incoming Cashier Settlement & Order Alert Banner */}
          {cashierAlert && (
            <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 sm:p-5 border-2 rounded-2xl shadow-xl animate-pulse ${
              cashierAlert.type === 'online_paid'
                ? 'bg-gradient-to-r from-emerald-500/20 via-emerald-500/10 to-emerald-500/5 border-emerald-500/70'
                : 'bg-gradient-to-r from-amber-500/20 via-amber-500/10 to-amber-500/5 border-amber-500/70'
            }`}>
              <div className="flex items-center gap-3.5">
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold shrink-0 shadow ${
                  cashierAlert.type === 'online_paid' ? 'bg-emerald-600 text-white' : 'bg-amber-500 text-black'
                }`}>
                ...
                <div>
                  <div className="text-base font-extrabold text-slate-900 flex items-center gap-2">
                    <span>
                      {cashierAlert.type === 'online_paid'
                        ? `Table #${cashierAlert.tableNumber} New Order #${cashierAlert.orderNumber}`
                        : `Table #${cashierAlert.tableNumber} Requested Bill Settlement`}
                    </span>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${
                      cashierAlert.type === 'online_paid' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-500 text-black'
                    }`}>
                      {cashierAlert.type === 'online_paid' ? 'Paid Online' : 'Action Required'}
                    </span>
                  </div>
                  <div className="text-xs text-slate-700 font-medium mt-0.5">
                    {cashierAlert.message} &nbsp;·&nbsp; Total: <strong className="text-slate-900 text-sm font-black">Rs. {cashierAlert.total.toLocaleString()}</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto">
                {cashierAlert.type === 'online_paid' ? (
                  <button
                    onClick={() => {
                      const matched = orders.find((o) => o.id === cashierAlert.orderId);
                      if (matched) setViewOrder(matched);
                      else setActiveSection('online_paid');
                      dismissCashierAlert();
                    }}
                ...
                <button
                  ...
                  className="px-3.5 py-2 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold text-xs rounded-xl transition"
                >
                  Dismiss
                </button>
              </div>
